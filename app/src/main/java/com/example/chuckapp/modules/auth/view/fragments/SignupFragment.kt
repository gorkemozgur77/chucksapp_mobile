...
class SignupFragment : Fragment() {
    ...
    private fun checkExistance(email : String) {
        ...
        authClient.getAuthApiService().checkEmailExistance(email).enqueue(object : retrofit2.Callback<EmailExitanceResponce>{
            override fun onResponse(call: Call<EmailExitanceResponce>, response: Response<EmailExitanceResponce>) {
                emailProgressbar.visibility = View.GONE
                if(response.isSuccessful){
                    println(response.body())
                    if(response.body()?.existance == "false"){
                        emailLayId.endIconDrawable = resources.getDrawable(R.drawable.ic_baseline_check_circle_24)
    ...
    private fun sendSignUpRequest(context: Context){
        ...
        authClient = AuthClient()
        authClient.getAuthApiService().signUp(getUserInfo(context)).enqueue(object : retrofit2.Callback<SignResponse>{
