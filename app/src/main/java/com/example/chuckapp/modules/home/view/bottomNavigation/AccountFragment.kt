...
import com.example.chuckapp.modules.home.view.bottomNavigation.recyclerAdapters.ActiveLogRecyclerAdapter
import com.example.chuckapp.modules.home.view.bottomNavigation.recyclerAdapters.InactiveLogRecyclerAdapter
...
class AccountFragment : Fragment() {
    ...
    private fun signOut(context: Context){
        HomeClient().getHomeApiService(context).signOut(SignOutRequest(Constants.PLATFORM_NAME)).enqueue(signOutResponseHangler)
    }

    private fun getActiveLog(context: Context)  {
        HomeClient().getHomeApiService(context).getActiveEntries().enqueue(getActiveLogResponseHandler)
    }

    private fun getInactiveLog(context: Context){
        HomeClient().getHomeApiService(context).getInactiveEntries().enqueue(getInactiveResponseHandler)
    }
    ...
    private fun updateInactiveLog(context: Context, url: String){
        ...
        HomeClient().getHomeApiService(context).getInactiveEntries(url).enqueue(updateInactiveLogResponseHandler)
    }
